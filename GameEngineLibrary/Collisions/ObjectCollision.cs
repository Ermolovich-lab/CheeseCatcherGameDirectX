...
namespace GameEngineLibrary.Collisions
{
    public abstract class ObjectCollision
    {
        ...
        public static bool IntersectsPoint(BoxCollision left, RayCollision right, out Vector3 point)
        {
            if (left == null || right == null)
            {
                point = Vector3.Zero;
            ...
            BoundingBox box = (BoundingBox)left.GetCollision();
            Ray ray = (Ray)right.GetCollision();

            var result = ray.Intersects(ref box, out point);
            return result;
        }
